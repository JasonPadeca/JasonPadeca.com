...
echo "\n=== Volunteer assignment: placing helpers in classes ==="
P -d postgres -tAc "drop database coop;" >/dev/null
P -d postgres -tAc "create database coop;" >/dev/null
for f in 00_supabase_stub.sql "$MIGRATIONS"/0*.sql 10_seed.sql; do
  P -d coop -f "$f" >/dev/null
done
P -d coop -f 90_volunteer_assignment.sql 2>&1 | strip

echo "\n=== Family accounts: the RLS boundary ==="
P -d postgres -tAc "drop database coop;" >/dev/null
P -d postgres -tAc "create database coop;" >/dev/null
for f in 00_supabase_stub.sql "$MIGRATIONS"/0*.sql 10_seed.sql; do
  P -d coop -f "$f" >/dev/null
done
P -d coop -f 95_family_accounts.sql 2>&1 | strip

echo "\n=== Absences: reporting, scoping, and the register ==="
P -d postgres -tAc "drop database coop;" >/dev/null
P -d postgres -tAc "create database coop;" >/dev/null
for f in 00_supabase_stub.sql "$MIGRATIONS"/0*.sql 10_seed.sql; do
  P -d coop -f "$f" >/dev/null
...
echo "\n=== Announcements, handouts, and classmates ==="
P -d postgres -tAc "drop database coop;" >/dev/null
P -d postgres -tAc "create database coop;" >/dev/null
for f in 00_supabase_stub.sql "$MIGRATIONS"/0*.sql 10_seed.sql; do
  P -d coop -f "$f" >/dev/null
...
P -d coop -f 94_registration_proposals.sql 2>&1 | strip
...
echo "\n=== Archiving a family: out of everything current, still in everything past ==="
P -d postgres -tAc "drop database coop;" >/dev/null
P -d postgres -tAc "create database coop;" >/dev/null
for f in 00_supabase_stub.sql "$MIGRATIONS"/0*.sql 10_seed.sql; do
  P -d coop -f "$f" >/dev/null
done
P -d coop -f 88_archive_family.sql 2>&1 | strip
