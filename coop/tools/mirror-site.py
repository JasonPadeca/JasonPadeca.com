...
# Photographs, not furniture.
#
# A page carries three kinds of image: the photograph somebody chose, the site
# logo, and tracking pixels. Only the first is worth offering to replace — the
# logo is an identity decision rather than a seasonal one, and a 43-byte GIF is
# not a picture of anybody.
#
# Judged on file size because that is what actually separates them here: the
# group photo is 259KB, the logo 3.6KB, the pixel 43 bytes.
MIN_IMAGE_BYTES = 20_000


def mark_images(html, page_dest):
    """Tag replaceable photographs with data-img, and collect them."""
    out, pos, n = [], 0, 0

    for m in re.finditer(r"<img\b[^>]*>", html, re.I):
        tag = m.group(0)
        src = re.search(r'src="([^"]+)"', tag)
        if not src or "data-img=" in tag:
            continue

        local = OUT / src.group(1)
        if not local.exists() or local.stat().st_size < MIN_IMAGE_BYTES:
            continue

        n += 1
        key = str(n)
        alt = (re.search(r'alt="([^"]*)"', tag) or [None, ""])[1]
        images.append((page_dest, key, src.group(1), alt, local.stat().st_size))

        out.append(html[pos:m.start()])
        out.append(tag[:-1].rstrip() + f' data-img="{key}">')
        pos = m.end()
...
def main():
    ...
    img_seed = OUT / "supabase" / "seed" / "site-images.sql"
    with img_seed.open("w", encoding="utf-8") as fh:
        fh.write("-- Generated by tools/mirror-site.py. Replaceable photographs\n"
                 "-- on the public pages. Safe to re-run.\n\n")
        q = lambda v: "'" + str(v).replace("'", "''") + "'"
        for page, key, src, alt, size in images:
            fh.write(
                "insert into public.site_images (page, img_key, original_src, alt, bytes)\n"
                f"values ({q(page)}, {q(key)}, {q(src)}, {q(alt)}, {size})\n"
                "on conflict (page, img_key) do update set original_src = excluded.original_src,\n"
                "  alt = excluded.alt, bytes = excluded.bytes;\n")
    print(f"images written: {len(images)} replaceable -> {img_seed}")
