...
export async function registration(app, params) {
  let rec, family;
  try {
    [rec, family] = await Promise.all([
      api.registrationRecord(params.family, params.semester),
      api.family(params.family).catch(() => null),
    ]);
  } catch (e) {
  ...
  render(app, `
    ...
    <div class="sheet">
      <header class="sheet-head">
        <div>
          <h1>${esc(s?.family?.display_name ?? family?.display_name ?? "Registration")}</h1>
      ...
      ${drift === null ? `<div class="sheet-note">
        ...
        snapshot of what the co-op held at the time.</div>` : ""}
      ...
      ${Array.isArray(drift) && drift.length ? `<div class="sheet-alert">
        ...
        <div class="tiny">A correction is a perfectly ordinary reason for this.
          It is shown so it cannot pass unnoticed.</div>
      </div>` : ""}
        ...
        <table class="sheet-table">
          <tbody>
            ...
            ${line("Payment received", rec.payment_received_at
              ? fmtDateTime(rec.payment_received_at) + (rec.payment_note ? ` — ${rec.payment_note}` : "")
              : "Not recorded")}
            ${line("Registered", rec.registered_at
              ? fmtDateTime(rec.registered_at) : "Not registered")}
            ${(rec.outstanding_at_registration ?? []).length
              ? line("Outstanding when registered",
                  rec.outstanding_at_registration.join(", "))
              : ""}
          </tbody>
        </table>
      </section>
      ...
      ${s ? `<section>
        <h2>The family, as recorded on ${esc(fmtDate(s.taken_at))}</h2>
        <table class="sheet-table">
          <tbody>
