  ...
  $("#edit").addEventListener("click", async () => {
    const v = await formDialog({
      ...
      fields: [
        ...
        { name: "primary_phone", label: "Primary phone", value: f.primary_phone,
          hint: "The number to ring. Printed on class rosters." },
...
async function childDialog(familyId, existing = null) {
  const v = await formDialog({
    ...
    fields: [
      ...
      { name: "email", label: "Student's own email", type: "email", value: existing?.email,
        hint: "Older students often have one. Appears on class rosters." },
      { name: "phone", label: "Student's own phone", value: existing?.phone,
        hint: "For students who carry a phone. Rosters show it beside their email." },
      { name: "allergies", label: "Allergies", value: existing?.allergies,
        placeholder: "Peanuts, bee stings",
        hint: "Allergies only — printed on every class roster. Leave blank if none." },
      { name: "medical_notes", label: "Medical notes", value: existing?.medical_notes,
        placeholder: "Type 1 diabetic, carries insulin",
        hint: "Medication and conditions a teacher may need to act on. Also printed on rosters." },
      { name: "notes", label: "Admin notes", type: "textarea", value: existing?.notes,
        hint: "Everything else. Not printed on rosters." },
    ],
  });
  if (!v) return;
  try {
